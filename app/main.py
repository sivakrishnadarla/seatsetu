...
from .db import (AuditLog, AttainmentRun, College, Competitor, Conversation, CounselorTask, PracticePass, FollowUpLog,
                 Course, CourseReg, EvidenceFile, EvidenceItem, Hostel, KeyDate, Lead, Message, MockInterview,
...
@app.post("/api/accred/evidence/{evidence_id}/upload")
async def evidence_upload(evidence_id: int, file: bytes = File(...), filename: str = Form("doc.pdf"),
                          db: Session = Depends(get_db)):
    """Upload the REAL document (scan/photo/PDF/Excel) for an AQAR evidence item.
    Stores it in the college's own vault; auto-marks the item collected."""
    e = db.query(EvidenceItem).get(evidence_id)
    if not e:
        raise HTTPException(404, "Evidence not found")
    if len(file) > 8 * 1024 * 1024:
        raise HTTPException(413, "File too large — max 8 MB per document")
    f = EvidenceFile(college_id=e.college_id, evidence_id=e.id,
                     filename=str(filename)[:200], content_type="application/octet-stream",
                     size=len(file), content=file)
    db.add(f)
    e.status = "collected"
    guardrails.log(e.college_id, "staff", "accred.evidence_uploaded",
                   {"id": e.id, "file": f.filename, "bytes": len(file)}, db)
    db.commit()
    return dict(ok=True, file_id=f.id, status=e.status)


@app.get("/api/accred/evidence/file/{file_id}")
def evidence_download(file_id: int, db: Session = Depends(get_db)):
    f = db.query(EvidenceFile).get(file_id)
    if not f:
        raise HTTPException(404, "File not found")
    return Response(f.content, media_type=f.content_type or "application/octet-stream",
                    headers={"Content-Disposition": f'attachment; filename="{f.filename}"'})


@app.delete("/api/accred/evidence/file/{file_id}")
def evidence_delete(file_id: int, db: Session = Depends(get_db)):
    f = db.query(EvidenceFile).get(file_id)
    if not f:
        raise HTTPException(404, "File not found")
    guardrails.log(f.college_id, "staff", "accred.evidence_file_deleted", {"id": f.id}, db)
    db.delete(f)
    db.commit()
    return dict(ok=True)


@app.post("/api/colleges/{college_id}/accred/evidence/seed-aqar")
def seed_aqar(college_id: int, db: Session = Depends(get_db)):
    """One tap: create evidence rows for all standard AQAR criteria that are missing."""
    from .accred import AQAR_ITEMS
    C(college_id, db)
    have = {e.criterion for e in db.query(EvidenceItem).filter_by(college_id=college_id).all()}
    made = 0
    for crit, desc in AQAR_ITEMS:
        if crit not in have:
            db.add(EvidenceItem(college_id=college_id, criterion=crit, title=desc))
            made += 1
    db.commit()
    return dict(ok=True, created=made)


@app.get("/api/colleges/{college_id}/accred/dossier.zip")
def dossier_zip(college_id: int, request: Request, db: Session = Depends(get_db)):
    """ONE-CLICK NAAC submission pack: attainment reports + evidence vault files +
    index + summary. The IQAC downloads this and uploads it on the NAAC portal."""
    a = authz.actor(request, db)
    if not a or a["role"] not in authz.SETTINGS_ROLES:
        raise HTTPException(403, "Principal or admin role required")
    import csv as _csv
    ...
    from .accred import AQAR_ITEMS
    col = C(college_id, db)
    ev = (db.query(EvidenceItem).filter_by(college_id=college_id)
            .order_by(EvidenceItem.criterion).all())
    files = (db.query(EvidenceFile).filter_by(college_id=college_id).all())
    runs = (db.query(AttainmentRun).filter_by(college_id=college_id)
              .order_by(AttainmentRun.id).all())
    paps = db.query(QPaper).filter_by(college_id=college_id).all()
    st = accred.aqar_status(ev)
    buf = _io.BytesIO()
    with _zf.ZipFile(buf, "w", _zf.ZIP_DEFLATED) as z:
        z.writestr("README.txt",
                   f"AQAR DOSSIER — {col.name}\n"
                   f"Generated by SeatSetu (Pooja Soft Solutions) on {utcnow():%Y-%m-%d %H:%M}\n"
                   f"Readiness: {st['readiness']}% ({sum(1 for i in st['items'] if i['status']=='collected')}/{st['total_items']} criteria collected)\n"
                   "\nContents:\n"
                   "  00_SUMMARY.csv          — index of every evidence item (criterion, owner, status, links)\n"
                   "  attainment/             — CO/PO attainment reports computed from marks Excel\n"
                   "  question_papers/        — Bloom-mapped model question papers (metric 2.6.3)\n"
                   "  evidence/<criterion>/   — the actual uploaded documents (scans, photos, PDFs)\n"
                   "\nThis dossier is PREPARED by SeatSetu. Submission to NAAC is done by the\n"
                   "college's own IQAC on the NAAC portal — under the college's own signature.\n")
        rows = [["criterion", "metric_description", "title", "owner", "status", "file_note", "uploaded_files"]]
        by_ev = {}
        for f in files:
            by_ev.setdefault(f.evidence_id, []).append(f)
        for crit, desc in AQAR_ITEMS:
            for e in [x for x in ev if x.criterion == crit]:
                names = "; ".join(f.filename for f in by_ev.get(e.id, []))
                rows.append([crit, desc, e.title, e.owner, e.status, e.file_note, names])
        for e in [x for x in ev if x.criterion not in {c for c, _ in AQAR_ITEMS}]:
            rows.append([e.criterion, "(custom item)", e.title, e.owner, e.status, e.file_note,
                         "; ".join(f.filename for f in by_ev.get(e.id, []))])
        out = _io.StringIO(); _csv.writer(out).writerows(rows)
        z.writestr("00_SUMMARY.csv", out.getvalue())
        for r in runs:
            safe = "".join(ch for ch in str(r.result.get("course", f"run{r.id}")) if ch.isalnum() or ch in " -_")[:60]
            rep = dict(r.result)
            lines = [f"Course: {rep.get('course')}", f"Students: {rep.get('students')}",
                     f"Target level: {rep.get('target')}", "",
                     "CO attainment (level 0-3):"]
            for co, v in (rep.get("co_attain") or {}).items():
                lines.append(f"  CO{co}: level {v.get('level')} · avg {v.get('avg_pct')}% · {v.get('students')} students")
            lines += ["", "PO attainment (direct):"]
            for po, v in (rep.get("po_direct") or {}).items():
                lines.append(f"  {po}: {v}")
            if rep.get("po_final"):
                lines.append("Final PO (with survey blend): " + ", ".join(f"{p}: {v}" for p, v in rep["po_final"].items()))
            if rep.get("gaps"):
                lines += ["", "Gap analysis & actions:"]
                for g in rep["gaps"]:
                    lines.append(f"  {g.get('po')} short by {g.get('gap')}: {g.get('action')}")
            z.writestr(f"attainment/run{r.id}_{safe}.txt", "\n".join(lines))
            out = _io.StringIO()
            w = _csv.writer(out)
            w.writerow(["CO", "level", "avg_pct", "students"])
            for co, v in (rep.get("co_attain") or {}).items():
                w.writerow([f"CO{co}", v.get("level"), v.get("avg_pct"), v.get("students")])
            z.writestr(f"attainment/run{r.id}_{safe}.csv", out.getvalue())
        for q in paps:
            safe = "".join(ch for ch in (q.title or f"qpaper{q.id}") if ch.isalnum() or ch in " -_")[:60]
            z.writestr(f"question_papers/qpaper{q.id}_{safe}.json", __import__("json").dumps(q.paper, indent=1, default=str))
        for f in files:
            e = db.query(EvidenceItem).get(f.evidence_id)
            folder = (e.criterion if e else "misc").replace("/", "-")
            safe = "".join(ch for ch in f.filename if ch.isalnum() or ch in " .-_") or f"file{f.id}"
            z.writestr(f"evidence/{folder}/{safe}", f.content)
    buf.seek(0)
    return Response(buf.getvalue(), media_type="application/zip",
                    headers={"Content-Disposition": f"attachment; filename=AQAR_dossier_{col.short or college_id}.zip"})
...
def list_evidence(college_id: int, db: Session = Depends(get_db)):
    ...
    by_ev = {}
    for f in files:
        by_ev.setdefault(f.evidence_id, []).append(dict(id=f.id, filename=f.filename, size=f.size))
    return [dict(id=e.id, criterion=e.criterion, title=e.title, owner=e.owner,
                 status=e.status, file_note=e.file_note, files=by_ev.get(e.id, [])) for e in rows]
